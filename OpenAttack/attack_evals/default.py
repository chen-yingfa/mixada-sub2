...
class DefaultAttackEval(AttackEval):
    ...
    * **clear:** Clear all the Accumulated results.
    ...
    def eval(self, dataset, total_len=None, visualize=False, save_dir=None, return_examples=False, targeted=True):
        """
        ...
        for data, x_adv, y_adv, info in (tqdm(self.eval_results(dataset, targeted=targeted), total=total_len) if self.__progress_bar else self.eval_results(dataset, targeted=targeted)):
            # print ("data.x:", data.x)
            # print ("x_adv:", x_adv)
    ...
    def eval_results(self, dataset, targeted=True):
        """
        ...
        # print("clsf_wrapper:", clsf_wrapper)
        for data in dataset:
            ...
            try:
                if targeted:
                    ...
                    res = self.attacker(clsf_wrapper, data.x)
            except:
                print ("Exception thrown by attacker")
                res = None 
            if res is None:
