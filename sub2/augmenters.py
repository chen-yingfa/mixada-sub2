...
class CParseAugmenter(Augmenter):
    """
    SUB2 + P + L + N
    Pick substitution by choosing another substructure with
    same label and same length.
    """
    def __init__(
...
class CParseLengthFreeAugmenter(CParseAugmenter):
    """
    SUB2 + P + L + N
    Pick substitution by choosing a substructure with same label, 
    then substitute using the leftmost n words (n is the length of
    the original substructure)
    """
...
class CParseSynonymAugmenter(CParseAugmenter):
    """
    Contextualized Substitution (CTX SUB)
    Mask one word then general a substituion word using BERT
    """
    def __init__(
                self, dataset, 
...
class CParseRandomAugmenter(CParseAugmenter):
    """
    Shuffle word order in a random sub structure
    """
    def __init__(self, dataset):
...
class Sub2Augmenter(CParseAugmenter):
    """
    SUB2
    same_len (+P): 
        Only swap substructures with the same number of words
    same_phrase_label (+L): 
        Only swap substructures with same phrase label 
        (NOTE: this assume that all given substructures are phrases)
    same_text_label (+T): 
        Only swap substructures that are from texts with 
        the same classification.
    senti:
        Only swap substructures with the same sentiment (as given in SST-2)
        (NOTE: this dataset must pass in spans with valid values for key 'senti')
    """
    def __init__(self, dataset, same_len=False, same_phrase_label=True, same_text_label=True, senti=False):
        super(Sub2Augmenter, self).__init__(dataset)
        self.same_len = same_len
        self.same_phrase_label = same_phrase_label
        self.same_text_label = same_text_label
    ...
    def build_example_table(self):
        ...
        for span in self.dataset:
            key = self._get_hash(span)
            example_table[key].append(span)
    ...
    def augment(self, size=None):
        ...
        while len(self.dataset.trees) < size:
            ...
            span = copy.deepcopy(random.choice(self.dataset))
            words = span['tree'].leaves()
            left, right = span['left'], span['right']
            span_label = span['span_label']
            tree_label = span['tree'].label()


            cand = self.get_substitution(span)
            if cand == span:
                continue
            cand = copy.deepcopy(cand)
            cand_words = cand['tree'].leaves()
            cand_left = cand['left']
            cand_right = cand['right']

            
            new_words = words[:left] + cand_words[cand_left:cand_right] + words[right:]
            new_tree = Tree(tree_label, new_words)
    ...
    def get_substitution(self, span):
        key = self._get_hash(span)
        return random.choice(self.example_table[key])

    def _get_hash(self, span):
        key = tuple()
        tree = span['tree']
        words = tree.leaves()
        text_label = tree.label()
        phrase_label = span['span_label']

        if self.same_len:
            key = (*key, len(span[0]))
        if self.same_phrase_label:
            key = (*key, phrase_label)
        if self.same_text_label:
            key = (*key, text_label)
        return key
...
class CParseLengthSpanAugmenter(CParseAugmenter):
    """
    SUB2 + P + L + N 
    Choose substitution by picking a substructure with same label,
    then replace a random range of words in the original substructure 
    with a random range of the same number of words in the chosen 
    candidate.
    """
    def __init__(self, dataset):
