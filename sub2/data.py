...
class PTBDataset(Dataset):
    def __init__(
        ...
        if isinstance(trees, str):
            ...
            # for path in glob(trees):
            for line in open(trees):
                tree, label = line.split('\t')
            ...
            print("Input to PTBDataset must be str or [Tree]")
            exit(0)
        # preproc spans
    ...
    def concat(self, other):
        ...
        self.spans.append(other.spans)

    def add_examples(self, examples):
        """
        examples: [(sent: str, label), ...]
        """
        orig_n_trees = len(self.trees)
        for sent, label in examples:
            ...
            tree.set_label(label)
            self.trees.append(tree)
        self.add_spans(self.trees[orig_n_trees:])
        ...
        if self.use_spans is True:
            for tree in trees:
                ...
                for span in spans:
                    ...
                    if right - left >= self.span_min_length and \
                        right - left < len(tree.leaves()):
                        self.spans.append({
                            'tree': tree, 
                            **span})
                ...
                self.spans.append(
                    {
                        'tree': tree.leaves(), 
                        'left': 0,
                        'right': len(tree.leaves()),
                        'span_label': tree.label()
                    }
                )
